...
class Rechteck:
    def __init__(self, ampl=1, tv=0.5, vs=0, offs=0, per=1):
        ...
        if vs > 0:
            self.Data = np.append(self.Data[1000*int(1-vs):1000],self.Data[0:1000*int(1-vs)])
        if vs < 0:
            self.Data = np.append(self.Data[int(abs(vs))*1000:1000],self.Data[0:1000*int(abs(vs))])
        if per > 1:
    ...
    def spektrum_rechteck(self):
        data = np.append(self.Data[500:1000], self.Data[0:500])
        self.Spek = np.fft.fft(data)
        self.Spek = np.fft.fftshift(self.Spek)
        self.Max = max(np.abs(self.Spek))
        self.SpekB = np.abs(self.Spek)/self.Max
        self.Spek[np.abs(self.Spek)<3] = 0+0j
        self.SpekP = np.angle(self.Spek)        
    ...
    def reko_rechteck(self, anzh=10):
        if (anzh > 50):
            print('Die Anzahl der Aufbaufunktionen wird auf 50 begrenzt.')
            anzh = 50
        plt.figure(figsize=(15,5))
        plt.subplot(121)
        t = np.linspace(0,1,1000)
        xges = self.Max*self.SpekB[500]*np.ones(1000)/1000
        plot(t,xges)
        for i in range(1,anzh):
             x = 2*self.Max*self.SpekB[500+i]*np.cos(2*pi*i*t+self.SpekP[500+i])/1000
             x = np.append(x[500:1000], x[0:500])
             plt.plot(t,x)
             plt.grid(True)
